 ...
 * Both methods may fail silently — a miss and an error are treated the same way.
 */
...
export function createDictionaryLoader({
   ...
   * at once share one request instead of racing each other past the cache.
   */
...
/** Fisher-Yates. Returns a new array, leaving the source deck untouched. */
